...
const insertStmt = sqlite.prepare(
  `INSERT INTO reviews (id, pub_id, user_id, pub_name, rating, created_at) VALUES (?, ?, ?, ?, ?, ?)`,
);
const updateStmt = sqlite.prepare(
  `UPDATE reviews SET rating = ?, pub_name = COALESCE(?, pub_name), created_at = ? WHERE pub_id = ? AND user_id = ?`,
);
...
export function addReview(
  ...
  if (isNew) {
    insertStmt.run(crypto.randomUUID(), pubId, userId, pubName ?? null, rating, new Date().toISOString());
  } else {
    updateStmt.run(rating, pubName ?? null, new Date().toISOString(), pubId, userId);
  }
